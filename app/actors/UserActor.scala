...
class UserActor @Inject()(@Assisted id: String, @Named("stocksActor") stocksActor: ActorRef)
  ...
  // Useful way to mark out individual actos with websocket request context information...
  ...
  // If this acotr is killed directly, stop anything that we started running explicitly.
  ...
  /**
    * Adds several stocks to the hub, by asking the stockes acotr for stocks.
    */
  ...
  private def addStock(stock: Stock): Unit = {
    ...
    // Set up a flow that will let us pull out a kill switch for this specific stock,
