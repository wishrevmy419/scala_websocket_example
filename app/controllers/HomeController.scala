...
class HomeController @Inject()(@Named("userParentActor") userParentActor: ActorRef,
  ...
  /**
    * Creates a websocket. `acceptOrResult` is preferable here becaouse it returns a
    ...
    case rh if sameOriginCheck(rh) =>
      wsFutureFlow(rh).map { flow =>
        ...
        case e: Exception =>
          ...
          val jsError = Json.obj("error" -> "Cannnot create websocket")
  ...
  private def wsFutureFlow(request: RequestHeader): Future[Flow[JsValue, JsValue, NotUsed]] = {
    // Use guice assited injection to instantiate and configure the child actor.
...
trait SameOriginCheck {
  ...
  /**
    ...
    * against Cross-Site WebSocket Hijacking as WebSocket does not impliment Same Origin Policy.
    *
